...
import com.mv.desafio.xpto.dtos.requisicoesDTO.CriarContaDto;
...
public class ContasController {
	...
	public ResponseEntity<List<RespostaContaDto>> getAll() {
		...
		// Lista de contas(DTO)
		List<RespostaContaDto> respostaContaList = contasList.stream().map(conta -> mapearConta(conta))
	...
	public ResponseEntity<RespostaContaDto> getById(@PathVariable Long id) {
		...
		// Retorna o método privado mapearConta que transforma uma Entidade Contas em
		// DTO
	...
	private RespostaContaDto mapearConta(Contas conta) {
		...
		List<RespostaMovimentacaoDto> listaMovimentacao = conta.getListaDeMovimentacoes().stream() // Converter a lista
																									// de Movimentações
				.map(movimentacao -> MapearCamposUtil.converterMovimentacao(movimentacao)).collect(Collectors.toList());
	...
	// Recebe um DTO e transforma em um Model do tipo Contas para persistir os dados
	// no banco
	@PostMapping
	public ResponseEntity<RespostaGenericaDto> create(@Valid @RequestBody CriarContaDto criarConta) {
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(new RespostaGenericaDto("Conta Criada com sucesso!"));
	}
